...
export default function DiscoveryView({backend,embeddedProjectId,embeddedManifestId}) {
  ...
  const designStudy=()=>action("design",async()=>{
    ...
    try {
      ...
      const response=await api.sendMessage(projectId,{
        request_id:requestId,study_intent:"discovery",source_run_id:baseline?.id||null,provider:null,model:null,agent_role:"explorer",auto_run:false,attachments:[],structure_id:null,
  ...
  const ask=next=>action("review",async()=>{const r=await (next?api.nextStudyProposal(studyId):api.reviewStudy(studyId));setReview(r);setNotice(next?"A metered proposal was prepared. No new simulation was started.":"Advisory review recorded in the research-world chat. Numerical data and pass/fail checks remain authoritative.");});
      ...
      <div hidden={tab!=="campaign"}>
        ...
        <div className="discStudyContent">{s?<>
          <section className="discoveryCard discStudyHeader"><div><span className="eyebrow">{s.recipe.strategy.replaceAll("_"," ")} · PROTOCOL {s.recipe_hash.slice(0,12)}</span><h2>{s.recipe.title}</h2><p>{s.recipe.hypothesis}</p><span className={`discBadge discBadge--${s.state}`}>{s.state.replaceAll("_"," ")}</span></div><div className="discActions">
            ...
            {terminalStudy(s.state)&&s.trials.length>0&&<><button className="button button--secondary" disabled={!!busy} onClick={()=>ask(false)}><Sparkles size={15}/>{busy==="review"?"Agent working…":"Request AI review"}</button><button className="button button--secondary" disabled={!!busy} onClick={()=>ask(true)}><GitBranch size={15}/>Propose next experiment</button></>}
