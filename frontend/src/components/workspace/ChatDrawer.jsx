...
import {INTENTS} from "@/lib/research";
import Link from "next/link";
import { ArrowDown, Bot, Check, Copy, FlaskConical, GripVertical, History, Maximize2, Minimize2, Paperclip, PanelRightClose, PanelRightOpen, Pencil, Plus, RotateCcw, Send, Settings, ShieldAlert, Sparkles, Square, Wallet, X } from "lucide-react";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { formatDate, statusLabel } from "@/lib/format";
import { api } from "@/lib/api";
import ChatMarkdown from "./ChatMarkdown";
import ConversationHistory from "./ConversationHistory";
const ROLES=[["builder","Builder"],["explorer","Explorer"],["falsifier","Falsifier"],["theorist","Theorist"],["biomolecular_architect","Biomolecular architect"],["qmmm_planner","QM/MM planner"]];
const ALLOWED=["txt","md","json","csv","toml","yaml","yml","pdb","sdf","mol","xyz"];
...
export default function ChatDrawer({containerRef,messages,providers,busy,error,project,projects,structures,selectedStructure,onStructureSelect,onProjectSelect,onNewProject,onRenameProject,onDeleteProject,onSend,onCancel,activeRequestId,findingsRun,onOpenFindings}){
  const {selection}=useModelSelection();
  const [collapsed,setCollapsed]=useState(false),[width,setWidth]=useState(34),[dragging,setDragging]=useState(false);
  const [historyOpen,setHistoryOpen]=useState(false),[role,setRole]=useState("builder"),[provider,setProvider]=useState("");
  const [intent,setIntent]=useState("experiment");
  const [assumptions,setAssumptions]=useState(true);
  const [autoRun,setAutoRun]=useState(false),[content,setContent]=useState(""),[attachments,setAttachments]=useState([]);
  ...
  const currentProvider=configured.find(p=>p.provider===provider)||configured[0];
  const blocked=Boolean(policy?.paused)||!configured.length;
  ...
  useEffect(()=>{if(!configured.some(p=>p.provider===provider))setProvider(configured[0]?.provider||"");},[configured,provider]);
  useEffect(()=>{
  ...
  const addFiles=async(files)=>{
    const next=[];let total=attachments.reduce((n,a)=>n+a.size_bytes,0);setLocalError("");
    try{for(const f of Array.from(files||[])){
      if(attachments.length+next.length>=8)throw Error("At most eight attachments per message.");
      if(!ALLOWED.includes(f.name.split(".").pop()?.toLowerCase()))throw Error(`${f.name}: use a supported text, data, or molecular structure file.`);
      if(f.size>2*1024*1024||total+f.size>8*1024*1024)throw Error("Attachment limit: 2 MiB per file, 8 MiB per message.");
      next.push({name:f.name,mime_type:f.type||"text/plain",size_bytes:f.size,content:await f.text()});total+=f.size;
    }}catch(e){setLocalError(e.message);}setAttachments(a=>[...a,...next]);
  };
  const submit=async(event,override=null)=>{
    ...
    if(busy||!project||(!text&&!files.length))return;
    if(blocked){setLocalError(policy?.paused?"Paid calls are paused. Resume them in Usage & cost.":"Save a provider key and active model in Settings first.");return;}
    ...
    try{await onSend({content:text,request_id:newId(),provider:selection.provider||currentProvider?.provider||null,model:selection.model||null,reasoning_effort:selection.reasoning_effort||null,
      agent_role:override?.agent_role||role,study_intent:override?.study_intent||intent,experiment_options:(override?.study_intent||intent)==="experiment"?{...DEFAULT_BUILD,assumptions_allowed:assumptions}:null,auto_run:["plan","review"].includes(override?.study_intent||intent)?false:autoRun,attachments:files,structure_id:selectedStructure?.id||null,
    ...
    }catch(e){if(lastProject.current===world){setContent(original.content||text);setAttachments(original.attachments);setBranch(original.branch);}}
  };
  const repeat=(message,index)=>{
    const source=message.role==="user"?message:[...messages.slice(0,index)].reverse().find(m=>m.role==="user");
    if(source)submit(null,{content:source.content,agent_role:message.agent_role||source.agent_role,reply_to_message_id:source.id,branch_from_message_id:source.id});
  };
  ...
  const edit=m=>{setContent(m.content);setBranch(m.id);setCollapsed(false);setTimeout(()=>textarea.current?.focus(),20);};
  ...
  return <aside className={`researchChat ${dragging?"researchChat--dragging":""}`} style={{width:`${width}%`}} aria-label="Research chat" onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={e=>{if(!e.currentTarget.contains(e.relatedTarget))setDragOver(false);}} onDrop={e=>{e.preventDefault();setDragOver(false);addFiles(e.dataTransfer.files);}}>
    ...
    <header className="researchChatHeader"><div className="researchIdentity"><Sparkles size={19}/><div><strong>{project?.name||"Research assistant"}</strong><small>{project?"Persistent conversation · evidence-linked":"Configure a provider to begin"}</small></div></div><div className="researchHeaderTools">
      ...
      <button type="button" title="New research world" aria-label="New research world" onClick={onNewProject} disabled={busy}><Plus size={17}/></button>
    ...
    {historyOpen&&<div className="researchHistoryOverlay"><div className="researchHistoryClose"><button type="button" onClick={()=>setHistoryOpen(false)}><X size={15}/>Close history</button></div><ConversationHistory projects={projects} activeId={project?.id} onSelect={id=>{if(!busy){onProjectSelect(id);setHistoryOpen(false);}}} onNew={()=>{if(!busy){onNewProject();setHistoryOpen(false);}}} onRename={onRenameProject} onDelete={id=>{if(!busy)onDeleteProject(id);}}/></div>}
    <div className="researchContext"><label className="pfIntent"><select aria-label="Research intent" value={intent} disabled={busy} onChange={e=>setIntent(e.target.value)}>{INTENTS.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label><details className="expRole"><summary>Role</summary><label><Bot size={15}/><select value={role} onChange={e=>setRole(e.target.value)} aria-label="Active research role" disabled={busy}>{ROLES.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select></label></details><Link href="/usage/" title="Token usage, cost controls, and stopping all calls"><Wallet size={14}/><span>Usage controls</span></Link></div>
    {intent==="experiment"&&<label className="expChatAssumptions"><input type="checkbox" checked={assumptions} onChange={e=>setAssumptions(e.target.checked)}/>Exploratory assumptions allowed · 180s / 1 GiB / 64 candidates</label>}
    ...
    <div className="researchTranscript" ref={transcript} onScroll={()=>{const el=transcript.current;const close=el.scrollHeight-el.scrollTop-el.clientHeight<90;bottom.current=close;setNearBottom(close);}}>
      {!messages.length&&<div className="researchChatEmpty"><span><Sparkles size={25}/></span><h3>{project?"What should we investigate?":"Your research starts here."}</h3><p>{project?"Build an experiment, inspect a structure, or challenge a result. Nothing is sent to a model until you press Send.":"Create a research world, add your own API key, and choose a model. No automatic questions or paid calls."}</p>{!configured.length?<Link href="/settings/" className="button button--secondary"><Settings size={15}/>Configure provider</Link>:<div className="researchSuggestions">{[["Design an experiment","builder"],["Challenge the current evidence","falsifier"],["Inspect the selected molecular structure","biomolecular_architect"]].map(([text,r])=><button key={r} type="button" onClick={()=>{setContent(text);setRole(r);}} disabled={!project}>{text}</button>)}</div>}</div>}
      {messages.map((m,index)=>{const user=m.role==="user";const meta=m.metadata||{};return <article key={m.id} className={`researchMessage ${user?"researchMessage--user":"researchMessage--assistant"} ${m.kind==="error"?"researchMessage--error":""}`}>
        ...
        <div className="researchMessageBody">{user&&<div className="userMessageMeta">YOU <time>{formatDate(m.created_at)}</time></div>}<ChatMarkdown>{m.content}</ChatMarkdown>{meta.attachments?.length>0&&<div className="researchMessageFiles">{meta.attachments.map((a,i)=><span key={a.id||i}><Paperclip size={12}/>{a.name}</span>)}</div>}</div>
        ...
        <footer className="researchMessageActions"><button type="button" onClick={()=>copy(m)} title="Copy message">{copied===m.id?<Check size={13}/>:<Copy size={13}/>}<span>{copied===m.id?"Copied":"Copy"}</span></button>{user&&<button type="button" onClick={()=>edit(m)} disabled={busy}><Pencil size={13}/><span>Edit & branch</span></button>}<button type="button" onClick={()=>repeat(m,index)} disabled={busy||blocked} title="Sends a new metered model request"><RotateCcw size={13}/><span>{user||m.kind==="error"?"Retry":"Regenerate"}</span></button>{!user&&meta.model&&<small>{meta.model}</small>}</footer>
      </article>;})}
      {busy&&<div className="researchProgress" role="status"><Bot size={18}/><div><strong>{phase?statusLabel(phase):"Working on this request…"}</strong><p>{phase==="repairing_proposal"?"Repairing only the rejected proposal within your configured budget.":phase==="validating_proposal"?"Checking the schema and numerical constraints before execution.":"A bounded model call. Stop remains available throughout the request."}</p><div className="progressPulse"><i/><i/><i/></div></div><button type="button" onClick={onCancel} disabled={!activeRequestId}><Square size={13}/>Stop</button></div>}
    </div>
    ...
    <div className="researchComposerArea">
      {blocked&&<div className="researchSetupHint">{policy?.paused?<>Paid calls paused. <Link href="/usage/">Resume in Usage & cost</Link></>:<>No provider ready. <Link href="/settings/">Save a key and choose a model</Link></>}</div>}
      ...
      <form className="researchComposer" onSubmit={submit}>
        ...
        <textarea ref={textarea} value={content} onChange={e=>{draftOwner.current=project?.id||null;setContent(e.target.value);}} placeholder={project?`Ask the ${ROLES.find(([v])=>v===role)?.[1]||"researcher"}…`:"Create a research world to start a conversation"} disabled={!project||busy} rows={3} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey&&!e.nativeEvent.isComposing){e.preventDefault();submit(e);}}}/>
        <div className="researchComposerToolbar"><div>
          <input type="file" multiple hidden ref={file} accept={ALLOWED.map(x=>`.${x}`).join(",")} onChange={e=>{addFiles(e.target.files);e.target.value="";}}/>
          <button className="attachButton" type="button" aria-label="Attach research files" title="Attach research files" disabled={!project||busy} onClick={()=>file.current?.click()}><Paperclip size={18}/></button>
          <ModelPicker providers={providers} disabled={busy} compact/>
          <label className="researchAutoRun" title="Explicitly run this one validated experiment after building. Does not authorize another model call or follow-on run."><input type="checkbox" checked={autoRun} onChange={e=>setAutoRun(e.target.checked)}/><span>Build & run</span></label>
        </div>{busy?<button type="button" className="researchSend researchSend--stop" aria-label="Stop active request" disabled={!activeRequestId} onClick={onCancel}><Square size={17}/></button>:<button type="submit" className="researchSend" aria-label="Send message" title="Send · Enter" disabled={!project||blocked||(!content.trim()&&!attachments.length)}><Send size={18}/></button>}</div>
      </form>
      <div className="researchComposerNote"><span>Enter to send · Shift+Enter for a new line</span><Link href="/usage/">{policy?`${fmtLimit(policy.max_output_tokens)} output cap · ${policy.repair_attempts} repair${policy.repair_attempts===1?"":"s"}`:"Usage & cost"}</Link></div>
    </div>
  </aside>;
}
