...
export default function LaboratoryActivity({jobs=[],onSelect,onControl}) {
  ...
  async function control(job,action){
    ...
    const budget=resumeBudgets[job.id];const options=action==='resume'&&budget?{time_limit_seconds:budget==='off'?null:Number(budget)}:{};
  ...
  return <section className={styles.activity} aria-label="Laboratory activity">
    <header className={styles.heading}><div><span className={styles.eyebrow}>LABORATORY ACTIVITY</span><h2>Research in progress</h2></div><span>{jobs.filter(job=>labJobPresentation(job).active).length} active · {jobs.length} saved</span></header>
    ...
    <div className={styles.jobs}>{ordered.map(job=>{
      ...
      return <article key={job.id} className={`${styles.job} ${view.active?styles.isActive:''}`}>
        ...
        {view.resumable&&job.kind!=='specialist'&&<label className={styles.meta}>{job.deadline_at&&Date.parse(job.deadline_at)<=now?'Original deadline expired. Choose a new maximum:':'Continuation budget:'}<select aria-label={`Continuation time for ${job.title||'job'}`} value={resumeBudgets[job.id]||''} onChange={event=>setResumeBudgets(values=>({...values,[job.id]:event.target.value}))}><option value="">Keep original budget / inherit active parent</option><option value="900">15 minutes</option><option value="3600">1 hour</option><option value="14400">4 hours</option><option value="off">Timer off</option></select>{job.parent_id&&<span>For a running parent, resume with its budget. Choose a new budget to continue independently after the parent has stopped.</span>}</label>}
        <RecordedActivity job={job}/>
        <footer className={styles.controls}>{job.kind==='export'&&view.state==='completed'&&<a href={`${api.baseUrl}${exportStatusFromJob(job).download_url}`} download={exportDownloadName(job)}><Download size={13}/>Save MP4</a>}{view.active&&onControl&&<><button type="button" onClick={()=>control(job,'pause')} disabled={busy}><Pause size={13}/>Pause</button><button type="button" onClick={()=>control(job,'cancel')} disabled={busy}><Square size={13}/>Stop</button></>}{view.resumable&&onControl&&<button type="button" onClick={()=>control(job,'resume')} disabled={busy}><Play size={13}/>{busy?'Resuming…':'Resume'}</button>}<button type="button" onClick={()=>onSelect?.(job)}>{view.state==='completed'?'Open results':'Inspect job'}<ArrowUpRight size={13}/></button></footer>
