...
export function installCameraNavigation(THREE,{camera,controls,element,span=1,onChange,fit}){
  ...
  const keydown=event=>{if(event.ctrlKey||event.metaKey||event.altKey||event.target&&event.target!==element)return;const actions={ArrowLeft:{orbit:[.12,0]},ArrowRight:{orbit:[-.12,0]},ArrowUp:{orbit:[0,-.12]},ArrowDown:{orbit:[0,.12]},'+':{zoom:-.12},'=':{zoom:-.12},'-':{zoom:.12},Home:{home:true}};let action=actions[event.key];if(event.shiftKey&&event.key.startsWith('Arrow'))action={pan:[event.key==='ArrowLeft'?-.08:event.key==='ArrowRight'?.08:0,event.key==='ArrowUp'?.08:event.key==='ArrowDown'?-.08:0]};if(action){event.preventDefault();event.stopPropagation();command(action);}};
