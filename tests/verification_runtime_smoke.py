...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='phaseforge-discovery-') as folder:
        ...
        environment=os.environ.copy();environment["PHASEFORGE_VERIFIER_PYTHON"]=str(pathlib.Path(os.sys.executable).resolve())
        ...
        with backend_process([str(binary),'--cpu-only','--config',str(config)], root/'backend.log', environment=environment) as proc:
            for _ in range(100):
                try:
