...
tokens('Molecular viewport and controls retained',read('frontend/src/components/workspace/MoleculeViewport.jsx')+read('frontend/src/components/workspace/MoleculeWorkbench.jsx'),['THREE.InstancedMesh','selectedAtomIds','Create region plan','Import structure']);
...
tokens('Viewer interpolation is display-only and time-based',viewer+sceneViewer,['createSceneViewer','OrbitControls','sampleFrames','interpolatePosition','requestAnimationFrame','ResizeObserver','webglcontextlost','poseStamp','renderer.dispose()','Playback interpolation affects the display only.']);
