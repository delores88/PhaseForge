...
test('staging verifies exact inputs before replacement and refuses extra or altered files',async()=>{
  ...
  try{
    ...
    const before=fs.readFileSync(path.join(destination,'runtime-seeds/science-v5/python.exe'));
    fs.writeFileSync(path.join(sourceRoot,'science-v5/extra.py'),'unexpected');await assert.rejects(stageSeeds(options),/differs/);
    assert.deepEqual(fs.readFileSync(path.join(destination,'runtime-seeds/science-v5/python.exe')),before);
    fs.unlinkSync(path.join(sourceRoot,'science-v5/extra.py'));fs.writeFileSync(path.join(sourceRoot,'science-v5/python.exe'),'modified');
    await assert.rejects(verifySeed(path.join(sourceRoot,'science-v5'),path.join(manifestRoot,'science-v5.manifest.json')),/differs/);
    assert.deepEqual(fs.readFileSync(path.join(destination,'runtime-seeds/science-v5/python.exe')),before);
