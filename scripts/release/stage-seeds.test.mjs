...
test('staging verifies exact inputs before replacement and refuses extra or altered files',async()=>{
  ...
  try{
    ...
    const before=fs.readFileSync(path.join(destination,'runtime-seeds/science-v2/python.exe'));
    fs.writeFileSync(path.join(sourceRoot,'science-v2/extra.py'),'unexpected');await assert.rejects(stageSeeds(options),/differs/);
    assert.deepEqual(fs.readFileSync(path.join(destination,'runtime-seeds/science-v2/python.exe')),before);
    fs.unlinkSync(path.join(sourceRoot,'science-v2/extra.py'));fs.writeFileSync(path.join(sourceRoot,'science-v2/python.exe'),'modified');
    await assert.rejects(verifySeed(path.join(sourceRoot,'science-v2'),path.join(manifestRoot,'science-v2.manifest.json')),/differs/);
    assert.deepEqual(fs.readFileSync(path.join(destination,'runtime-seeds/science-v2/python.exe')),before);
