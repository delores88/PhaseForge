...
test('staging verifies exact inputs before replacement and refuses extra or altered files',async()=>{
  ...
  try{
    ...
    const before=fs.readFileSync(path.join(destination,'runtime-seeds/science-v4/python.exe'));
    fs.writeFileSync(path.join(sourceRoot,'science-v4/extra.py'),'unexpected');await assert.rejects(stageSeeds(options),/differs/);
    assert.deepEqual(fs.readFileSync(path.join(destination,'runtime-seeds/science-v4/python.exe')),before);
    fs.unlinkSync(path.join(sourceRoot,'science-v4/extra.py'));fs.writeFileSync(path.join(sourceRoot,'science-v4/python.exe'),'modified');
    await assert.rejects(verifySeed(path.join(sourceRoot,'science-v4'),path.join(manifestRoot,'science-v4.manifest.json')),/differs/);
    assert.deepEqual(fs.readFileSync(path.join(destination,'runtime-seeds/science-v4/python.exe')),before);
