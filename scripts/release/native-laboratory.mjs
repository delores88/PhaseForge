...
export function createLaboratoryAcceptance({page,call,remember,observedProcesses,workspace,output,projectId,sourceCommit,version,backendSha256}){
  ...
  async function capture(job,label){
    ...
    if(job.kind==='generated'){
      ...
      const processObservation=solverProcess(job,observedProcesses(),path.join(workspace,'environments/science-v5/python.exe'));state.processes??={};state.processes[job.id]=processObservation;write(path.join(directory,'process-observation.json'),{job_id:job.id,event:job.events.find(event=>event.kind==='solver_started'),process:processObservation});
  ...
  // Capture is also callable with a retained fixture transport so unit tests
  // exercise the same decoder and receipt boundary. It never launches a job.
  return {firstLaunch,beforeQuit,reopen,captureRetainedJob:capture,evidence:state};
}
