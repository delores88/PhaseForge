...
export function createLaboratoryAcceptance({page,call,remember,observedProcesses,workspace,output,projectId,sourceCommit,version,backendSha256}){
  ...
  async function capture(job,label){
    ...
    if(job.kind==='generated'){
      ...
      const process=solverProcess(job,observedProcesses(),path.join(workspace,'environments/science-v3/python.exe'));state.processes??={};state.processes[job.id]=process;write(path.join(directory,'process-observation.json'),{job_id:job.id,event:job.events.find(event=>event.kind==='solver_started'),process});
