...
test('passing acceptance requires every real lifecycle phase, new numerical engines and recorded playback',()=>{const state={first_launch_passed:true,cancel:{passed:true},quit:{observed_active:true},restart:{passed:true},restored:{passed:true,index:3},checks:{openmm:{passed:true},field:{passed:true},heat:{passed:true,playback:{passed:true}},flow:{passed:true,playback:{passed:true}},boundary:{passed:true}}};assert.equal(acceptanceComplete(state),true);for(const key of ['first_launch_passed','cancel','quit','restart','restored','checks']){const next=structuredClone(state);delete next[key];assert.equal(acceptanceComplete(next),false);}for(const key of ['openmm','field','heat','flow','boundary']){const next=structuredClone(state);delete next.checks[key];assert.equal(acceptanceComplete(next),false);}for(const key of ['heat','flow']){const next=structuredClone(state);delete next.checks[key].playback;assert.equal(acceptanceComplete(next),false);}state.restored.index=2;assert.equal(acceptanceComplete(state),false);});
test('solver PID matching uses creation time and bundled path, rejecting stale PID and host Python',()=>{const executable=path.resolve('fixture/environments/science-v5/python.exe'),job={created_at:'2026-09-12T01:00:00Z',events:[{kind:'solver_started',at:'2026-09-12T01:00:01Z',data:{pid:42,python:executable}}]},row={pid:42,created:Date.parse('2026-09-12T01:00:00.500Z')/1000,exe:executable};assert.deepEqual(solverProcess(job,[row],executable),row);assert.throws(()=>solverProcess(job,[{...row,created:row.created-20}],executable));assert.throws(()=>solverProcess(job,[{...row,exe:path.resolve('host/python.exe')}],executable));assert.throws(()=>solverProcess(job,[row,row],executable));});
