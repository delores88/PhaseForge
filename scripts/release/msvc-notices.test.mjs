...
test('exact Microsoft notices bind the copied files to the frozen science-v5 DLL and upstream archive',async t=>{
  const {notices}=fixture(t),result=await verifyMsvcNotices(notices);
  assert.equal(result.seed_kind,'science-v5');assert.equal(result.files.length,4);assert.equal(result.outside_asar,true);
  assert.equal(result.origin.destination,'runtime/runtime-seeds/science-v5/MSVCP140.dll');
