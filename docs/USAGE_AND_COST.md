...
proposal call, repair and connection test has a separate record under its parent
request ID. Records contain provider/model, project ID, purpose, attempt, status,
...
If the response arrives but the proposal fails schema or numerical validation,
...
checks the returned manifest against the trusted runtime before persistence/execution.
