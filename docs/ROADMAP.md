...
[compute/recovery](COMPUTE_AND_RECOVERY.md).
...
The current engines are explicit ODE and classical particle solvers. Priorities for
...
solver backend, finer checkpoint boundaries for integrators, and isolated worker
processes with enforceable memory limits. Extend allocation to multiple GPUs only
