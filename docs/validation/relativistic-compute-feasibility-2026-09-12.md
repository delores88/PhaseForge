...
## Subsequent separately authorized black-hole foundation

After the gauge stage closed, minimal GSL provisioning and a separate pinned TwoPunctures/Z4c build were authorized and completed. One explicitly admitted low-momentum initial-data/evolution attempt then ran under a fixed 600-second deadline, stopped with its process group drained, and retained complete metric/constraint states at times zero and 0.5126953125. It did not reach its time-one target or establish a common horizon, merger, converged numerical accuracy or physical boost. The original gauge binary/results remain unchanged. Full input, build, failed-layout, resource, numerical-growth and incomplete-run evidence is recorded in [the black-hole pilot plan and result](relativistic-black-hole-pilot-plan-2026-09-12.md), including a read-only private CUDA-toolkit assessment. The exact 0.999c requirement remains unfulfilled.
