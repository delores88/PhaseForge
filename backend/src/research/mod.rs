...
#[derive(Debug,Clone,Serialize,Deserialize)]pub struct Source {pub id:String,pub title:String,pub authors:String,pub year:String,#[serde(default)]pub publication_date:String,pub doi:String,pub abstract_text:String,pub url:String,pub scope:String}
...
pub fn context(db:&Database,id:Uuid)->anyhow::Result<Value>{
    ...
    let mut ids=HashSet::new();let sources=searches.iter().filter(|s|s.status=="completed").flat_map(|s|s.results.iter())
        .filter(|r|ids.insert(r.id.clone())).take(15).map(|r|json!({"id":r.id,"title":short(&r.title,700),"year":r.year,"publication_date":r.publication_date,"doi":r.doi,"abstract_excerpt":short(&r.abstract_text,1800),"scope":r.scope})).collect::<Vec<_>>();
...
pub async fn retrieve(query:&str)->anyhow::Result<(Vec<Source>,String,Option<u64>)>{
    ...
    let client=reqwest::Client::builder().timeout(std::time::Duration::from_secs(25)).redirect(reqwest::redirect::Policy::none()).user_agent(concat!("PhaseForge/",env!("CARGO_PKG_VERSION")," research-workspace")).build()?;
    let mut response=client.get("https://www.ebi.ac.uk/europepmc/webservices/rest/search").query(&[("query",query),("format","json"),("resultType","core"),("pageSize","15")]).send().await?;
    if !response.status().is_success(){bail!("Europe PMC returned HTTP {}",response.status());}
...
pub fn parse_sources(v:&Value)->anyhow::Result<Vec<Source>>{
    ...
    for row in rows.iter().take(15){let text=|key:&str|row[key].as_str().unwrap_or("");let source=text("source");let external=text("id");
        ...
        let abstract_text=plain(text("abstractText"),16000);out.push(Source{id,title:plain(text("title"),1200),authors:plain(text("authorString"),2000),year:text("pubYear").into(),publication_date:plain(text("firstPublicationDate"),10),doi:text("doi").chars().take(300).collect(),url:format!("https://europepmc.org/article/{source}/{external}"),scope:if abstract_text.is_empty(){"metadata only"}else{"abstract excerpt and metadata; full text not read"}.into(),abstract_text});
    }Ok(out)
}
