...
async fn fixture(mode:&'static str)->Fixture{
    ...
    let app=Router::new().route("/v1/responses",post(move |Json(body):Json<Value>|{let captured=captured.clone();let arrived=arrived_signal.clone();let release=release_signal.clone();async move{
        let number={let mut calls=captured.lock();calls.push(body);calls.len()};
...
fn update(text:&str)->SteeringRequest{SteeringRequest{request_id:Uuid::new_v4(),content:text.into(),attachments:vec![]}}
...
#[tokio::test]async fn in_flight_proposal_is_superseded_before_its_visual_side_effect(){
    ...
    let calls=f.calls.lock();assert_eq!(calls.len(),2);assert!(calls[1]["input"].to_string().contains("Do not recolor"));assert_eq!(calls[1]["model"],"gpt-6-astra");assert_eq!(calls[1]["reasoning"]["effort"],"low");assert_eq!(job.deadline_at,None);
}
#[tokio::test]async fn final_response_cannot_complete_over_a_saved_unanswered_update(){
    ...
    let completed=finished(&f,job.id).await;assert_eq!(completed.state,"completed");assert_eq!(f.calls.lock().len(),2);assert!(completed.events.iter().any(|event|event.kind=="continuing_for_update"));assert_eq!(completed.events.iter().filter(|event|event.kind=="completed").count(),1);
    ...
    receive(&f.state,job.id,request).unwrap();assert!(receive(&f.state,job.id,update("A later question")).unwrap_err().downcast_ref::<SessionFinished>().is_some());assert_eq!(f.calls.lock().len(),2);
}
#[tokio::test]async fn solver_wait_yields_to_an_update_without_rewriting_or_stopping_the_solver(){
    ...
    let source=f.state.laboratory.get(f.source).unwrap();assert_eq!(source.state,"queued");assert_eq!(source.input["parameters"]["original"],true);assert_eq!(f.calls.lock().len(),2);
}
#[tokio::test]async fn message_identity_conflicts_roll_back_the_update_receipt(){
    ...
    assert!(receive(&f.state,parent.id,SteeringRequest{request_id:original.id,content:"Replace the old answer".into(),attachments:vec![]}).is_err());
