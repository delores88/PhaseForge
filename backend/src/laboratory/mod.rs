...
impl LaboratoryService {
    ...
    async fn ensure_environment(&self,id:Uuid,token:&CancellationToken)->anyhow::Result<PathBuf> {
        ...
        self.ensure_science_attempt_identity(id)?;
        self.event(id,"provisioning","Verifying and copying the bundled scientific runtime. No host Python or network installation is used.",json!({"runtime":"science-v5"}))?;
