...
#[cfg(test)]mod tests{
    ...
    #[test]fn science_v4_refuses_old_runtime_resume_before_any_mutation_and_keeps_artifacts_readable(){
        for prior in ["science-v2","science-v3"] {
        ...
        assert!(!service.executing(child.id));assert!(!service.config.data_directory.join("environments/science-v4").exists());
    ...
    #[test]fn science_v4_matching_receipt_resumes_and_generated_runtime_path_is_v3(){
        let (_dir,service,parent,child)=fixture();
        ...
        assert_eq!(service.environment_python(),service.config.data_directory.join("environments/science-v4/python.exe"));
        assert_eq!(runtime::RuntimeKind::Generated.directory(&service.config.data_directory),service.config.data_directory.join("environments/python-numpy-v3/runtime"));
